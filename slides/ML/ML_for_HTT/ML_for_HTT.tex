...
\begin{frame}{Model's performances}
...
\begin{equation*}
r = \frac{\text{prediction}}{\text{true value}} = \frac{\mml}{m_{\higgsML}}
\end{equation*}
\submanip Closer to 1 is better.
\submanip $r=\num{1.00}\pm{0.05}$ from \num{80} to \SI{800}{\GeV}
\submanip \higgsML\ mass reconstruction \textbf{achieved {\color{ltcolorgreen}\OK}}
...
\begin{tikzpicture}[overlay, remember picture]
\draw [ltcolorred] (.2\textwidth, .7\textheight) node {\Large DNN's \mml} ;
\draw [ltcolorred] (.75\textwidth, .7\textheight) node {\Large\mTtot} ;
\end{tikzpicture}
\end{frame}
