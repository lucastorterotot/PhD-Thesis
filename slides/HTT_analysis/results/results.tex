...
\only<2->{\manip MSSM \Higgs\ with $m=\SI{400}{\GeV}$ expected \tikzmarknode{signal.}{nodesignal}}
...
\only<5->{\manip Data/Bkg agreement $\to$ \textbf{exclusion limits}.}
\end{minipage}
\hfill
\begin{minipage}[t]{.49\textwidth}
\begin{center}
...
\foreach \txt in {Karlsruhe Institute of Technology (DE), Imperial College (UK), DESY (DE), IP2I (FR)}{
\draw (A.south west) node [below right] (A) {\submanip \txt};
}
}
\end{tikzpicture}
...
\begin{minipage}[c]{.49\textwidth}
\begin{center}
\only<1-4>{\plotHTTshapes{mt_tot}{mssm_classic}{2018}{tt}{32}{prefit}}
\only<5>{\plotHTTModelIndepLimits{mssm_classic}{combined}{bbH}{_cmb}}
\only<6->{\plotHTTModelIndepLimits{mssm_classic}{combined}{bbH}{_cmb_vs_CMS_ATLAS}}
\end{center}
\end{minipage}
...
\draw [ltcolorred, very thick, latex-] (11.45, 5.1) --+ (0, .4);
}

\only<3>{
\draw [ltcolorred, very thick] (11.45,4.95) ellipse (.3 and .6) ;
\draw [ltcolorred, very thick, -latex] (11.15,4.95) to [out=-180, in = 0] (3.56,4) ;
}
