...
% General
\def\NordreNNT{2021LYSE1118}
\def\LYCEN{T 2021-0\todo{??}}
...
\newcommand{\prefiring}{pré-déclenchement}
...
\newcommand{\refApHTTimpactsLETTER}{I}
...
\newcommand{\refApHTTimpacts}{\ifref{annexe-impacts-HTT}{\ref{annexe-impacts-HTT}}{\refApHTTimpactsLETTER}}
...
% HTT
\newcommand{\HAtoTauTau}{\ensuremath{\Higgs/\HiggsA\to\tau\tau}}
...
\newcommand{\EMBcolor}{ltcolororange2}
\newcommand{\FAKEScolor}{ltcolorgreen2}
\newcommand{\TTBARcolor}{ltcolorviolet2}
\newcommand{\ZLLcolor}{ltcolorblue2}
\newcommand{\DIBcolor}{ltcolorbrown}
...
\def\localcats{}
\ifthenelse{\equal{#2}{mssm_vs_sm_h125}\or\equal{#2}{mssm_vs_sm}\or\equal{#2}{mssm_vs_sm_heavy}\or\equal{#2}{mssm_vs_sm_CPV-new_categories}}{
  \def\localcats{\CATsm+\CATbsm}
}{}
\ifthenelse{\equal{#2}{mssm_vs_sm_classic}\or\equal{#2}{mssm_vs_sm_CPV}}{
  \def\localcats{\CATbsm\ only}
}{}
...
\draw (2.35, 7) node [below left, text width=1cm] {\centering\scriptsize \WorkInProgress\par};

% cats
\draw (2.35, 6.6) node [right] {\scriptsize \localcats};
\end{tikzpicture}%
}
...
\ifthenelse{\equal{#7}{prefit}\and #6 < 30}{
...
\foreach \x in {40, 70}{
 \draw [thick, dashed, ltcolorred] ({1.25 + (\x-0)/(160-0)*(7.68-1.25)}, 2.8) --+ (0, 5.6-2.8);
 \draw [thick, dashed, ltcolorred] ({1.25 + (\x-0)/(160-0)*(7.68-1.25)}, 0.95) --+ (0, 2.2-0.95);
}

\draw ({1.25 + (20-0)/(160-0)*(7.68-1.25)}, 5.6) node {\tiny\color{ltcolorred} \CATtightmt};
\draw ({1.25 + (55-0)/(160-0)*(7.68-1.25)}, 5.6) node {\tiny\color{ltcolorred} \CATloosemt};
\draw ({1.25 + (115-0)/(160-0)*(7.68-1.25)}, 5.6) node {\tiny\color{ltcolorred} DR \Wjets};
\end{tikzpicture}%
}
...
\foreach \x in {-35, -10, 30}{
 \draw [thick, dashed, ltcolorred] ({1.25 + (\x+200)/(200+200)*(7.68-1.25)}, 2.8) --+ (0, 5.6-2.8);
 \draw [thick, dashed, ltcolorred] ({1.25 + (\x+200)/(200+200)*(7.68-1.25)}, 0.95) --+ (0, 2.2-0.95);
}

\draw ({1.25 + (-117.5+200)/(200+200)*(7.68-1.25)}, 5.6) node [below] {\tiny\color{ltcolorred} CR \ttbar};
\draw ({1.25 + (-22.5+200)/(200+200)*(7.68-1.25)}, 5.6+.25) node [left, rotate=90] {\tiny\color{ltcolorred} \CATlowdz};
\draw ({1.25 + (20+200)/(200+200)*(7.68-1.25)}, 5.6+.25) node [left, rotate=90] {\tiny\color{ltcolorred} \CATmediumdz};
\draw ({1.25 + (115+200)/(200+200)*(7.68-1.25)}, 5.6) node [below] {\tiny\color{ltcolorred} \CAThighdz};
\end{tikzpicture}%
}
...
\newcommand{\TauHdz}{$d_z < \SI{0.2}{\centi\meter}$ avec $d_z$ la distance longitudinale entre la trace principale du \tauh\ et le vertex primaire}
...
\newcommand{\FromPairMatchToHLTObjects}{de la paire sélectionnée doivent correspondre, le cas échéant, aux objets ayant activé un des \HLTpaths\ utilisés pour enregistrer l'événement.
...
\newcommand{\TransverseMassWjetsInfosTXT}{Cette coupure permet de s'assurer que la région de signal soit orthogonale à la région de détermination des \fakefactors\ des événements \Wjets\ tout en rejetant ce bruit de fond dans la région de signal. Les facteurs de faux sont abordés dans la section~\ref{chapter-HTT_analysis-section-bg_estimation-FF_method}.}
