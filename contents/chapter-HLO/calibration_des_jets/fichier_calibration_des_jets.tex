\section{Calibration en énergie des jets dans CMS}\label{chapter-HLO-section-CMS}
...
\end{itemize}
La réponse d'un jet est définie comme
\begin{equation}
R = \frac{\pT}{\pT_\ptcl}
\mend
\label{eq-def_R_jet}
...
\par La JEC est déterminée pour chaque année de prise de donnée et pour différentes subdivisions de ces années.
En effet, les conditions expérimentales changent au cours du temps.
...
Par exemple, entre 2016 et 2017, la partie centrale du trajectographe est remplacée par un système d'architecture différente~\cite{CMS-TDR-11,cms_trk_upgrade_2017}.
De 2017 à 2018, les photodiodes des \CMSendcaps\ du HCAL sont remplacées par des photomultiplicateurs à silicium~\cite{CMS-TDR-10,SiPM_CMS_conf}.
...
C'est en particulier le cas pour la réponse du détecteur dont la correction, présentée dans la section~\ref{chapter-HLO-section-CMS-subsec-reponse}, est réalisée après la correction de l'empilement abordée ci-après.
\subsection{Correction de l'empilement}\label{chapter-HLO-section-CMS-subsec-PU}
Des contributions additionnelles à l'énergie et à l'impulsion des jets peuvent apparaître du fait de l'empilement, décrit dans le chapitre~\refChLHCCMS.
...
\label{eq-chapter-HLO-section-CMS-subsec-PU-offset_corr_v2}
\end{equation}
...
\mend[,]
\label{eq-chapter-HLO-section-CMS-subsec-PU-pTaddptcl}
\end{equation}
avec
$\pT_\ptcl^\text{avec PU}$ et $\pT_\ptcl^\text{sans PU}$
les impulsions du jet au niveau particule avec et sans empilement.
La contribution additionnelle de l'empilement au niveau particule est alors paramétrée en fonction de $\pT_\reco^\text{CHS}$, $\eta$, $A$ et $\rho$ et la correction de l'empilement résiduel $\mathcal{C}_\text{PU}$ définie par~\eqref{eq-chapter-HLO-section-CMS-subsec-PU-offset_corr_v2} peut se réécrire
\begin{equation}
\mathcal{C}_\text{PU}(\pT_\reco^\text{CHS}, \eta, A, \rho)
= 1 - \frac{\left[\rho_0(\eta) + \rho\,\beta(\eta)(1+\gamma(\eta)\log\pT_\reco^\text{CHS})\right]A}{\pT_\reco^\text{CHS}}
\label{eq-chapter-HLO-section-CMS-subsec-PU-offset_corr_v1}
\end{equation}
où $\rho_0(\eta)$, $\beta(\eta)$ et $\gamma(\eta)$ sont les paramètres de cette correction, dépendants de $\eta$.
\par La figure~\ref{fig-chapter-HLO-section-CMS-subsec-PU-JERC_RunI-Figure_005} montre $\average{\pT_\ptcl^\text{add}}$ en fonction de l'impulsion transverse du jet au niveau particule, avant et après application de la correction de l'empilement.
Les résultats de la figure~\ref{subfig-chapter-HLO-section-CMS-subsec-PU-JERC_RunI-Figure_005b} sont cohérents avec l'absence d'énergie supplémentaire due à l'empilement à $\pm\SI{0.2}{\GeV}$ jusqu'à $\pT_\ptcl = \SI{500}{\GeV}$ et à $\pm\SI{0.6}{\GeV}$ au-delà.
...
\begin{figure}[h]
\centering
\subcaptionbox{Avant correction.\label{subfig-chapter-HLO-section-CMS-subsec-PU-JERC_RunI-Figure_005a}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/from_JERC_RunI/Figure_005-a.tex}}
\hfill
\subcaptionbox{Après correction.\label{subfig-chapter-HLO-section-CMS-subsec-PU-JERC_RunI-Figure_005b}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/from_JERC_RunI/Figure_005-b.tex}}
\caption[Contribution additionnelle de l'empilement au niveau particule.]{Contribution additionnelle de l'empilement au niveau particule telle que définie dans l'équation~\eqref{eq-chapter-HLO-section-CMS-subsec-PU-pTaddptcl} pour $\abs{\eta}<\num{1.3}$ en fonction de l'impulsion du jet au niveau particule pour différentes valeurs du nombre d'interaction d'empilement ($\mu$)~\cite{JERC_RunI}.}
\label{fig-chapter-HLO-section-CMS-subsec-PU-JERC_RunI-Figure_005}
\end{figure}
...
La méthode RC consiste à placer aléatoirement des cônes couvrant l'ensemble de l'acceptance du détecteur dans le plan $(\eta, \phi)$.
La reconstruction des jets se fait alors cône par cône, par regroupement des particules contenues dans le cône.
%La méthode RC reconstruit ainsi autant de jets que possible.
...
\mend
\end{equation}
Il est alors possible de définir un facteur d'échelle à appliquer aux paramètres $\rho_0$ et $\beta$ de l'équation~\eqref{eq-chapter-HLO-section-CMS-subsec-PU-offset_corr_v1} lorsque cette correction est appliquée aux données. Ce facteur d'échelle s'exprime
\begin{equation}
...
\subsection{Correction de la réponse du détecteur en $\pT$ et en $\eta$}\label{chapter-HLO-section-CMS-subsec-reponse}
...
\subsection{Corrections résiduelles}\label{chapter-HLO-section-CMS-subsec-residuals}
...
\mend
\label{eq-chapter-HLO-section-CMS-subsec-residuals-Cres_def}
\end{equation}
...
\mend
\label{eq-chapter-HLO-section-CMS-subsec-residuals-pT_bal_ppe1}
\end{equation}
L'objet de référence étant fidèlement reconstruit,
\begin{equation}
\vpT_\reco^\text{réf} \simeq \vpT_\ptcl^\text{réf} = \vpT^\text{réf}
\mend
\label{eq-chapter-HLO-section-CMS-subsec-residuals-pT_ref_approx_ptcl_reco}
\end{equation}
Ainsi, l'équation~\eqref{eq-chapter-HLO-section-CMS-subsec-residuals-pT_bal_ppe1} peut se réécrire à l'aide de~\eqref{eq-chapter-HLO-section-CMS-subsec-residuals-pT_ref_approx_ptcl_reco} sous la forme
\begin{equation}
\vpT^\text{réf} + \vpT_\ptcl^\text{jet} = \vec{0}
\mend
\label{eq-chapter-HLO-section-CMS-subsec-residuals-pT_bal_ppe2}
\end{equation}
...
\mend
\label{eq-chapter-HLO-section-CMS-subsec-residuals-pT_bal_ppe3}
\end{equation}
Il en résulte
\begin{equation}
\Rbal (\pT, \eta) = \frac{\pT_\reco^\text{jet}}{\pT^\text{réf}}
\label{eq-chapter-HLO-section-CMS-subsec-residuals-Rbal_def}
\end{equation}
...
\mend
\label{eq-chapter-HLO-section-CMS-subsec-residuals-pT_MPF_ppe1}
\end{equation}
Au niveau reconstruit, l'énergie transverse manquante (MET) doit être prise en compte dans le recul.
...
\mend
\end{equation}
En appliquant~\eqref{eq-chapter-HLO-section-CMS-subsec-residuals-pT_ref_approx_ptcl_reco} et~\eqref{eq-chapter-HLO-section-CMS-subsec-residuals-pT_MPF_ppe1} à l'équation précédente, il est possible d'écrire
\begin{equation}
...
\end{equation}
\end{description}
\subsubsection{Correction résiduelle relative en $\eta$}\label{chapter-HLO-section-CMS-subsec-residuals_eta}
\begin{wrapfigure}[18]{R}{.45\textwidth}
\centering
...
Cette correction s'appuie sur la bonne reconstruction des jets dans le \CMSbarrel.
...
\subsubsection{Correction résiduelle absolue en \pT}\label{chapter-HLO-section-CMS-subsec-residuals_pT}
...
Cette responsabilité s'inscrit dans la continuité de ce savoir-faire au sein du groupe CMS de l'IP2I, la mesure de cette correction en 2016 et 2017 y étant déjà réalisée~\cite{Hugues_thesis}.
La phénoménologie de ces événements ainsi que leur analyse sont détaillées dans les sections~\ref{chapter-HLO-section-pheno-GJets} et~\ref{chapter-HLO-section-JES}.
%une fois le ECAL calibré (test de presque chaque cristal en faisceau), calibration du HCAL.
\subsubsection{Correction résiduelle de saveur}\label{chapter-HLO-section-CMS-subsec-residuals_flavor}
...
\label{fig-chapter-HLO-section-CMS-subsec-PU-JERC_RunI-Figure_030a}
\end{wrapfigure}
...
et est représentée sur la figure~\ref{fig-chapter-HLO-section-CMS-subsec-PU-JERC_RunI-Figure_030a}.
...
\subsection{Incertitude sur la correction en énergie des jets}\label{chapter-HLO-section-CMS-subsec-unc}
...
Cette incertitude prend en compte l'échelle en énergie des objets de référence utilisés pour obtenir la correction résiduelle absolue en \pT\ décrite dans la section~\ref{chapter-HLO-section-CMS-subsec-residuals_pT} ainsi que les effets de l'ISR (\emph{Initial State Radiation}) et du FSR (\emph{Final State Radiation}) abordés dans la section~\ref{chapter-HLO-section-pheno-GJets}.
...
Cette incertitude est principalement due à la JER qui s'applique à l'objet de référence dans la correction résiduelle relative en $\eta$ décrite dans la section~\ref{chapter-HLO-section-CMS-subsec-residuals_eta} ainsi qu'aux effets de l'ISR et du FSR.
...
%\end{equation}
\subsection{Correction de la résolution en énergie}\label{chapter-HLO-section-CMS-subsec-JER}
...
Sa mesure est réalisée à l'aide d'événements \Gjets\ et \Zjets\ et les résultats obtenus lors du Run~I sont présentés sur la figure~\ref{subfig-chapter-HLO-section-CMS-subsec-JER-JERC_RunI-Figure_036-b}.
...
Les résultats obtenus lors du Run~II sont présentés sur la figure~\ref{subfig-chapter-HLO-section-CMS-subsec-JER-JER_SF_RunII}.
Le principe est le même que pour les corrections résiduelles décrites dans les sections~\ref{chapter-HLO-section-CMS-subsec-residuals_eta} et~\ref{chapter-HLO-section-CMS-subsec-residuals_pT}. Au lieu de s'intéresser à la moyenne de la distribution, c'est sa largeur qui est étudiée.
\begin{figure}[h]
\centering
\subcaptionbox{JER en fonction de \pT\ dans le \CMSbarrel\ de CMS ($\abs{\eta}<\num{1.3}$) pour différentes valeurs d'interactions d'empilement $\mu$ lors du Run~I~\cite{JERC_RunI}.\label{subfig-chapter-HLO-section-CMS-subsec-JER-JERC_RunI-Figure_036-b}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/from_JERC_RunI/Figure_036-b.png}}
\hfill
\subcaptionbox{Facteurs d'échelle de la résolution en énergie des jets en fonction de $\eta$ lors du Run~II~\cite{CMS-DP-2020-019}.\label{subfig-chapter-HLO-section-CMS-subsec-JER-JER_SF_RunII}}[.45\textwidth]
...
\label{fig-chapter-HLO-section-CMS-subsec-JER}
\end{figure}
