\section{Correction résiduelle absolue en \pT\ avec les événements \Gjets}\label{chapter-HLO-section-JES}
...
\subsection{Événements utilisés}\label{chapter-HLO-section-JES-subsec-evt_select}
...
Le critère d'identification des photons retenu dans l'analyse est le critère strict, introduit dans le chapitre~\refChLHCCMS.
Il est demandé que $R_9 > \num{0.90}$, où $R_9$ est définie dans le chapitre~\refChLHCCMS.
...
%La figure~\ref{fig-chapter-HLO-section-pheno-GJets-photon_resolution}, page~\pageref{fig-chapter-HLO-section-pheno-GJets-photon_resolution}, montre en effet que ces photons possèdent une résolution relative en énergie de l'ordre de \SI{1}{\%}, contre environ \SI{2.5}{\%} pour les photons des \CMSendcaps.
...
Toutefois, la résolution en énergie des photons, représentée en fonction de $\abs{\eta}$ sur la figure~\ref{fig-chapter-HLO-section-pheno-GJets-photon_resolution} en page~\pageref{fig-chapter-HLO-section-pheno-GJets-photon_resolution}, est de l'ordre de \SI{1}{\%} dans le \CMSbarrel\ contre \SI{2.5}{\%} dans les \CMSendcaps.
...
Avec ce photon doit être présent au moins un jet reconstruit à l'aide de l'algorithme anti-\kT~\cite{Cacciari_antikT} avec un paramètre $R=\num{0.4}$ et respectant les critères définis dans le tableau~\ref{tab-chapter-HLO-section-jets_reco-subsec-jetID-2018} pour les données de 2018 et ceux du tableau~\ref{tab-chapter-HLO-section-jets_reco-subsec-jetID-2017UL} pour les données de 2017-UL.
Ces critères permettent de rejeter les jets issus du bruit de fond avec une efficacité de \SI{99}{\%}.
\par Les jets ainsi sélectionnés sont calibrés en énergie en suivant la procédure décrite dans la section~\ref{chapter-HLO-section-CMS} jusqu'à la correction résiduelle relative en $\eta$ incluse. Ils sont alors triés par impulsion transverse décroissante.
...
\end{table}
\subsection{Analyse}\label{chapter-HLO-section-JES-subsec-analyse}
\paragraph{Intervalles de $\pT^{\photon}$}
L'analyse a pour but de déterminer la correction résiduelle absolue en \pT\ des jets, définie dans la section~\ref{chapter-HLO-section-CMS-subsec-residuals}.
...
\mend
\label{eq-chapter-HLO-definition_alpha}
\end{equation}
...
L'évolution des réponses moyennes en fonction de $\alpha$ y étant linéaire \aposteriori, ce qui se retrouve dans les résultats de la figure~\ref{fig-chapter-HLO-section-JES-subsec-analyse-responsebal_and_MPF_eta0013_ptPhot_175_230_extrap}, page~\pageref{fig-chapter-HLO-section-JES-subsec-analyse-responsebal_and_MPF_eta0013_ptPhot_175_230_extrap}, ce choix rend possible une extrapolation simple vers $\alpha=0$.
\begin{table}[h]
\centering
\begin{tabular}{ccccc}
\toprule
...
Une extrapolation vers $\alpha=0$ est réalisée afin de s'affranchir de l'activité additionnelle des jets décrite dans la section~\ref{chapter-HLO-section-pheno-GJets-subsec-effets_radiatifs}.
Les intervalles de $\alpha$ utilisés pour la JES sont présentés dans le tableau~\ref{tab-alpha_intervalles}.
L'utilisation des ces intervalles inclusifs permet une extrapolation linéaire en $\alpha$, ce qui est réalisé sur la figure~\ref{fig-chapter-HLO-section-JES-subsec-analyse-responsebal_and_MPF_eta0013_ptPhot_175_230_extrap}.
\begin{figure}[h]
\centering
\subcaptionbox{Réponse balancée.\label{subfig-chapter-HLO-section-JES-subsec-analyse-response_eta0013_ptPhot_175_230_extrap}}[.45\textwidth]
...
\hfill
\subcaptionbox{Réponse MPF.\label{subfig-chapter-HLO-section-JES-subsec-analyse-responseMPF_eta0013_ptPhot_175_230_extrap}}[.45\textwidth]
...
\label{fig-chapter-HLO-section-JES-subsec-analyse-responsebal_and_MPF_eta0013_ptPhot_175_230_extrap}
\end{figure}
\subsection{Résultats}\label{chapter-HLO-section-JES-subsec-results}
La correction à appliquer aux données, définie par la formule~\eqref{eq-chapter-HLO-section-CMS-subsec-residuals-Cres_def} d'après la démarche exposée dans la section~\ref{chapter-HLO-section-CMS-subsec-residuals}, s'obtient en calculant la valeur moyenne des réponses \Rbal\ ou \RMPF\ pour les données et les simulations dans chacun des intervalles
...
\par Les résultats ainsi obtenus à l'aide des méthodes de la balance et MPF, avant et après extrapolation vers $\alpha=0$, sont présentés dans les sections~\ref{chapter-HLO-section-JES-subsec-results-subsubsec-before_extrap} et~\ref{chapter-HLO-section-JES-subsec-results-subsubsec-after_extrap}.
...
La dépendance en $\pT$ de la correction est déterminée grâce à un ajustement global réalisé avec les résultats d'autres analyses, présenté dans la section~\ref{chapter-HLO-section-JES-subsec-results-subsubsec-global_fit}.
Enfin, une vérification de la bonne mise en œuvre de la correction ainsi déterminée est présentée dans la section~\ref{chapter-HLO-section-JES-subsec-results-subsubsec-L2L3Res_cross_check}.
\subsubsection{Résultats avant extrapolation}\label{chapter-HLO-section-JES-subsec-results-subsubsec-before_extrap}
...
de \SI{3}{\%} pour $\abs{\eta^\text{jet}} > \num{2.5}$,
\par Il est à noter que ces résultats sont obtenus avant extrapolation vers $\alpha=0$. Or, cette extrapolation a un effet beaucoup plus important sur la réponse balancée que sur la réponse MPF, comme le montre la figure~\ref{fig-chapter-HLO-section-JES-subsec-analyse-responsebal_and_MPF_eta0013_ptPhot_175_230_extrap}.

\input{\PhDthesisdir/contents/chapter-HLO/JES/figures_shortcuts/responses_balancing_alpha_0_3_2018ABCD.tex}
\input{\PhDthesisdir/contents/chapter-HLO/JES/figures_shortcuts/responses_MPF_alpha_0_3_2018ABCD.tex}
\input{\PhDthesisdir/contents/chapter-HLO/JES/figures_shortcuts/responses_balancing_extrapolated_2018ABCD.tex}
\input{\PhDthesisdir/contents/chapter-HLO/JES/figures_shortcuts/responses_MPF_extrapolated_2018ABCD.tex}

\subsubsection{Résultats après extrapolation}\label{chapter-HLO-section-JES-subsec-results-subsubsec-after_extrap}
L'extrapolation des réponses vers $\alpha=0$ est réalisée comme expliqué dans la section~\ref{chapter-HLO-section-JES-subsec-analyse}.
...
\input{\PhDthesisdir/contents/chapter-HLO/JES/figures_shortcuts/responses_recap_table.tex}

\subsubsection{Ajustement global}\label{chapter-HLO-section-JES-subsec-results-subsubsec-global_fit}
...
\item les incertitudes de la JEC, décrites section~\ref{chapter-HLO-section-CMS-subsec-unc}, page~\pageref{chapter-HLO-section-CMS-subsec-unc};
...
\end{itemize}
\par La figure~\ref{fig-chapter-HLO-section-JES-subsec-results-response_eta0013_alpha_0_3_2018} compare les résultats produits lors de ma thèse avec les événements \Gjets\ à ceux de l'analyse basée sur les événements \Zmmjets\ pour l'année 2018.
...
\begin{figure}[h]
\centering
\subcaptionbox{Avec les événements \Gjets.\label{subfig-chapter-HLO-section-JES-subsec-results-response_eta0013_alpha_0_3_Gjets_2018}}[.45\textwidth]
...
\hfill
\subcaptionbox{Avec les événements \Zmmjets.\label{subfig-chapter-HLO-section-JES-subsec-results-response_eta0013_alpha_0_3_Zmmjets_2018}}[.45\textwidth]
...
\label{fig-chapter-HLO-section-JES-subsec-results-response_eta0013_alpha_0_3_2018}
\end{figure}
...
\subsubsection{Test d'intégrité}\label{chapter-HLO-section-JES-subsec-results-subsubsec-L2L3Res_cross_check}
...
\input{\PhDthesisdir/contents/chapter-HLO/JES/figures_shortcuts/responses_recap_table_L2L3Res_cross_check.tex}
\begin{figure}[h]
\centering
