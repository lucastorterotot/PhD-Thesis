\section{Correction de la résolution en énergie avec les événements \Gjets}\label{chapter-HLO-section-JER}
...
La sélection des événements est ainsi faite comme dans le cas de la JES décrite section~\ref{chapter-HLO-section-JES-subsec-evt_select}, à ceci près que la correction résiduelle absolue en \pT\ des jets est appliquée.
\subsection{Définition de la résolution en énergie des jets}\label{chapter-HLO-section-JER-subsec-JER_definition}
La résolution en énergie des jets se détermine à l'aide de leur réponse balancée \Rbal.
À partir de la définition de \Rbal\ en page~\pageref{eq-chapter-HLO-section-CMS-subsec-residuals-Rbal_def}, il est possible d'écrire dans le cas des événements \Gjets
\begin{equation}
\Rbal
...
\subsection{Analyse}\label{chapter-HLO-section-JER-subsec-analyse}
\paragraph{Similitudes avec l'analyse menée pour la JES}
L'analyse des événements \Gjets\ dans le cas de la JER est semblable à celle pour la JES, décrite dans la section~\ref{chapter-HLO-section-JES-subsec-analyse}.
...
Une extrapolation vers $\alpha=0$ est réalisée afin de s'affranchir de l'activité additionnelle des jets décrits dans la section~\ref{chapter-HLO-section-pheno-GJets-subsec-effets_radiatifs}.
...
L'extrapolation est illustrée sur la figure~\ref{fig-chapter-HLO-section-JER-subsec-analyse-resolution_eta0005_ptPhot_700_3000_extrap}.
...
Puis, un ajustement constant en \pT\ des rapports de $\sigma_\text{JER}$ extrapolés à $\alpha=0$ entre données et simulations est déterminé dans chaque intervalle de $\eta^\text{jet}$, comme sur la figure~\ref{fig-chapter-HLO-section-JER-subsec-analyse-resolution_eta0005_extrapolated}.
La valeur de cet ajustement correspond au facteur à appliquer à la JER dans l'intervalle de $\eta^\text{jet}$.
\begin{figure}[h]
\centering
\subcaptionbox{Extrapolation vers $\alpha=0$ de la résolution des jets pour $\abs{\eta}<\num{0.52}$ et $\num{700}<\pT^{\photon}<\SI{3000}{\GeV}$ en 2018.\label{fig-chapter-HLO-section-JER-subsec-analyse-resolution_eta0005_ptPhot_700_3000_extrap}}[.45\textwidth]
...
\hfill
\subcaptionbox{Résolution en énergie des jets extrapolée à $\alpha=0$ pour $\abs{\eta}<\num{0.52}$ en 2018.\label{fig-chapter-HLO-section-JER-subsec-analyse-resolution_eta0005_extrapolated}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/JERC/JER/Run2018ABCD/extrapolated/resolution_eta0005_balancing_extrap.tex}}
...
\subsection{Résultats}\label{chapter-HLO-section-JER-subsec-results}
Les résultats issus de l'analyse des événements \Gjets\ pour l'année 2018 sont présentés sur la figure~\ref{fig-chapter-HLO-section-JER-subsec-results-fine_eta_binning_Scale_factor_res_vs_ETA}.
La combinaison avec l'analyses des événements dijet permet d'obtenir les facteurs correctifs utilisés par la collaboration, présentés sur la figure~\ref{fig-chapter-HLO-section-JER-subsec-results-CMS-DP-2020-019-JER_SF_RunII}.
Ces facteurs sont de l'ordre de \num{1.2} dans le \CMSbarrel\ et peuvent atteindre \num{2.3} dans les \CMSendcaps.
\begin{figure}[h]
\centering
\subcaptionbox{Facteurs correctifs déterminés avec les événements \Gjets\ en 2018.\label{fig-chapter-HLO-section-JER-subsec-results-fine_eta_binning_Scale_factor_res_vs_ETA}}[.45\textwidth]
...
\hfill
\subcaptionbox{Facteurs correctifs utilisés par la collaboration lors du Run~II~\cite{CMS-DP-2020-019}.\label{fig-chapter-HLO-section-JER-subsec-results-CMS-DP-2020-019-JER_SF_RunII}}[.45\textwidth]
...
\begin{figure}[h]
\centering
\subcaptionbox{Avant application des JER SF.\label{subfig-distrib_Gjets_18_resp_balancing_eta2530_ptPhot_175_230_only_L2Res_without_JER}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/JERC/distributions/2018/with_header/JERC_application/resp_balancing_eta2530_ptPhot_175_230_without_JER.tex}}
\hfill
\subcaptionbox{Après application des JER SF.\label{subfig-distrib_Gjets_18_resp_balancing_eta2530_ptPhot_175_230_only_L2Res_with_JER}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/JERC/distributions/2018/with_header/JERC_application/resp_balancing_eta2530_ptPhot_175_230_with_JER.tex}}
