...
\begin{figure}[h]
\centering
\includegraphics[width=.5\textwidth]{/home/torterotot/Documents/PhD-Thesis/contents/chapter-MS-MSSM/formalisme/light_hadrons_masses.png}
...
\end{figure}
\par La valeur de $g_s$ à une échelle d'énergie $k$ est reliée à la valeur de $g_s$ à une échelle d'énergie $\mu$ par la relation~\cite{salam2010elements}
\begin{equation}
g_s(k) = \frac{g_s(\mu)}{1+ \frac{11n_c-2n_f}{12\pi} g_s(\mu)\ln(\frac{k^2}{\mu^2})}
\end{equation}
avec $n_c$ le nombre de couleurs et $n_f$ le nombre de saveurs de quarks, \ie\ $n_c=3$ et $n_f=6$. Cette relation peut ainsi se réécrire
\begin{equation}
g_s (k) =
