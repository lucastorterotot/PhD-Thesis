...
Pour concevoir une analyse de physique des particules à même de tester le MSSM, il faut dans un premier temps déterminer la manifestation de ce modèle à observer.
Comme cela a été développé dans la section précédente, le MSSM implique l'existence de quatre bosons de Higgs supplémentaires dont deux neutres, \Higgs\ et \HiggsA.
S'ils existent, un signal leur correspondant doit pouvoir être observé.
...
Or, ces deux paramètres sont reliés par les équations~\eqref{eq-Higgs_mixing_angle_MSSM}, donnant
\begin{equation}
...
Dans ce cas, \Higgs\ et \HiggsA\ sont de masses similaires et \higgs\ prend le rôle du boson de Higgs du modèle standard observé expérimentalement en 2012~\cite{ATLAS_Higgs_discovery,CMS_Higgs_discovery,CMS_Higgs_discovery_2013}.
...
Alors, dans cette limite, $\alpha \sim \beta$ ou $\alpha\sim\beta\pm\frac{\pi}{2}$.
...
Les couplages du tableau~\ref{tab-Higgs_couplings_2HDM} deviennent alors ceux du tableau~\ref{tab-Higgs_couplings_MSSM_decoupling}.
\begin{table}[h]
\centering
\begin{tabular}{rccc}
\toprule
...
\begin{itemize}
\item \higgs\ se comporte comme le boson de Higgs du modèle standard, ce qui le rend tout à fait cohérent avec les observations actuelles;
\item \Higgs\ et \HiggsA\ présentent peu voire aucun couplage aux bosons vecteurs, par exemple la désintégration $\HiggsA\to\Zboson\Zboson$ est impossible mais $\HiggsA\to\Zboson\higgs$ est possible;
\item \Higgs\ et \HiggsA\ sont couplés de manières similaires aux fermions.
\end{itemize}
...
La production et les désintégrations de \Higgs\ et \HiggsA\ sont donc intrinsèquement liées à la présence de fermions d'isospin faible bas.
\subsection{Production de bosons de Higgs}\label{chapter-MS-MSSM-section-pheno_Higgs_MSSM-subsec-production}
La production de bosons de Higgs au LHC peut être réalisée selon plusieurs modes dont un calcul précis des sections efficaces a pu être fait dans le cadre du modèle standard~\cite{Higgs_xsec_book_1,Higgs_xsec_book_2,Higgs_xsec_book_3,Higgs_xsec_book_4}.
...
Les processus correspondant à ces différents modes sont présentés dans la section~\ref{chapter-MS-MSSM-section-pheno_Higgs_MSSM-subsec-production-SM} ci-après.
...
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-Higgs_VBF_u.tex}\vspace{\baselineskip}}
...
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-Higgs_VH_Z.tex}\vspace{\baselineskip}}
\hfill
...
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-Higgs_gg_loop_Zh.tex}\vspace{\baselineskip}}
...
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-Higgs_with_t_qq_g_tth.tex}\vspace{\baselineskip}}
\hfill
...
Le mode de production principal du boson de Higgs du modèle standard \higgs\ au LHC est la fusion de gluon.
Près de \SI{85}{\%} des bosons de Higgs sont produits ainsi.
...
Or, comme cela est vu dans la section~\ref{chapter-MS-MSSM-section-formalisme-subsec-Higgs_mechanism-subsubsec-fermions} le couplage du Higgs aux fermions est proportionnel à leur masse.
...
Bien que la section efficace du VBF soit dix fois plus faible que celle du \gluon\gluon\higgs, les deux quarks de l'état final donnent deux jets très caractéristiques, dont le processus de formation à partir des quarks est abordé dans la section~\ref{chapter-MSSM-formation_jets}.
...
Une fusion de gluons peut également amener à la production d'un Higgs en association avec un \Zboson, c'est le cas du processus de la figure~\ref{subfig-fgraph-Higgs_gg_loop_Zh}.
...
Dans ces cas, des jets sont également formés.
Le mode \ttbar\higgs\ est illustré sur la figure~\ref{fig-fgraph-Higgs_prod_with_t}.
Ces processus contribuent peu à la production de bosons de Higgs au LHC dans le cadre du modèle standard.
...
Il s'agit toujours du mode dominant si $\tan\beta$ ne prend pas de valeur élevée.
...
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-Higgs_VBF_t-MSSM.tex}\vspace{\baselineskip}}
...
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-Higgs_VH_W-MSSM.tex}\vspace{\baselineskip}}
\hfill
\subcaptionbox{Production en association avec un boson \Zboson.\label{subfig-fgraph-Higgs_VH_Z-MSSM}}[.45\textwidth]
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-Higgs_VH_Z-MSSM.tex}\vspace{\baselineskip}}
...
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-gg_hHAbb.tex}\vspace{\baselineskip}}

\vspace{2\baselineskip}
\subcaptionbox{\label{subfig-fgraph-bb_hHA}}[.45\textwidth]
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-bb_hHA.tex}\vspace{\baselineskip}}
\hfill
\subcaptionbox{\label{subfig-fgraph-bg_b_bhHA}}[.45\textwidth]
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/H_production/fgraph-bg_b_bhHA.tex}\vspace{\baselineskip}}
...
\end{figure}
\subsection{Désintégrations des bosons de Higgs}\label{chapter-MS-MSSM-section-pheno_Higgs_MSSM-subsec-desintegration_Higgs}
\begin{figure}[p]
\centering
...
Il est donc impossible d'observer directement la présence d'un boson de Higgs, comme cela peut se faire avec d'autres particules plus stables comme les kaons, les muons, les électrons ou les protons par exemple.
Pour étudier ces bosons, il faut donc observer leurs produits de désintégration.
...
\par
Dans la limite découplée du MSSM,
les couplages des bosons de Higgs supplémentaires \Higgs\ et \HiggsA\ 
aux bosons vecteurs sont supprimés
et ceux
aux fermions sont proportionnels:
\begin{itemize}
\item aux masses de ces derniers;
\item à $1/\tan\beta = \cot\beta$ pour ceux d'isospin faible haut;
\item à $\tan\beta$ pour ceux d'isospin faible bas.
\end{itemize}
Les rapports de branchement qui en découlent sont représentés sur la figure~\ref{fig-BR_hHA_tanbeta_5_30} pour les trois bosons de Higgs neutres du MSSM et pour $\tan\beta=5$ et $30$.
...
\par
Bien que le canal de désintégration $\higgs,\Higgs,\HiggsA \to \quarkb\antiquarkb$ possède un rapport de branchement 5 à 10 fois supérieur à celui du canal $\higgs,\Higgs,\HiggsA \to \antitau\leptau$, il est sujet à de nombreuses sources de bruit de fond au LHC où les collisions ont lieu entre protons.
C'est pour cette raison que cette thèse s'intéresse à la recherche de bosons de Higgs supplémentaires de haute masse se désintégrant en paire de~\tau, dont l'accessibilité expérimentale est meilleure.
...
Toutefois, les~\tau\ ne sont pas des particules stables et se désintègrent avant d'entrer dans les parties sensibles du détecteur.
Seuls leurs produits de désintégration sont observés.
\begin{figure}[h]
\vspace{\baselineskip}
\centering
\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/fgraph-H-tautau_small.tex}
\vspace{\baselineskip}
...
\end{figure}
\subsection{Désintégrations des leptons~\tau}\label{chapter-MS-MSSM-section-pheno_Higgs_MSSM-subsec-desintegration_lepton_tau}
La durée de vie des leptons~\tau\ est de \SI{290}{\femto\second},
ce qui est trop court pour qu'ils puissent se propager jusqu'aux parties sensibles du détecteur.
Un~\tau\ n'est donc pas directement observé, seuls ses produits de désintégration le peuvent.
\par Les leptons~\tau\ se désintègrent par interaction faible selon $\leptau\to\Wbosonminus\nutau$~\cite{PDG_booklet_2020}.
...
Le tableau~\ref{subtab-tau_decay_modes_BRs} résume plus en détail les rapports de branchement des différents modes de désintégration du~\tau.
\begin{figure}[h]
\centering
\vspace{\baselineskip}
...
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/tau_decays/fgraph-tau_to_tauh.tex}\vspace{\baselineskip}}
...
Il existe donc six canaux différents pour les événements dans lesquels un boson de Higgs se désintègre en paire de~\tau, listés avec leurs rapports de branchement dans le tableau~\ref{subtab-HTT_channels_BRs}.
