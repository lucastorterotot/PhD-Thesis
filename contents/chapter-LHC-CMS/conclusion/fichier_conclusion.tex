...
\par Une simulation d'événements physiques et du détecteur lui-même permettent d'obtenir une estimation des observations attendues pour un modèle théorique donné.
...
La comparaison des données réelles à celles simulées permet de tester le modèle standard et des modèles allant au-delà comme l'extension supersymétrique minimale du modèle standard (MSSM, \emph{Minimal Supersymmetric extension of Standard Model}) détaillée dans le chapitre~\refChMSSM.
Le chapitre~\refChHTT\ présente une analyse expérimentale menée pour la recherche d'un boson de Higgs supplémentaire prédit par ce modèle.