\subsection{Reconstruction et identification des particules}\label{chapter-LHC-section-evt_reco-subsec-ptc_ID}
...
\subsubsection{Muons}
\paragraph{Reconstruction de l'objet physique}
...
\end{equation}
\paragraph{Identification des muons}
...
\paragraph{Identification des électrons}
Deux critères d'identification des électrons existent.
Le premier est issu d'une analyse multivariée (\emph{MultiVariate Analysis}, MVA), le second est obtenu à partir de coupures (\emph{cuts}) sur certaines variables.
\subparagraph{\EleIDMVA}
Ce critère est basé sur un arbre de décision (\emph{Bossted Decision Tree}, BDT) \cite{cmsElectronMVA}.
...
\paragraph{Identification des photons}
