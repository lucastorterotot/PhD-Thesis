...
Ces désintégrations sont accompagnées de jets, comme exposé dans le chapitre~\refChMSSM.
...
De plus, un algorithme (\emph{kink finder}) sépare la trajectoire du muon et détermine un $\chi^2$ afin de vérifier si cette trajectoire reconstruite peut en réalité provenir de deux traces réelles distinctes.
...
%\item $H/E_{SC}$ le rapport de l'énergie hadronique dans un cône de $\Delta R < \num{0.15}$ autour du photon sur l'énergie du \emph{supercluster}.
...
%La formation des jets ainsi que l'hadronisation sont détaillées dans le chapitre~\refChMSSM.
