...
{\input{\PhDthesisdir/plots_and_images/Feynman_diagrams/fgraph-H-tautau_mutau-for_mTtot.tex}}
...
\end{figure}
\par
Toutefois, l'énergie transverse manquante, introduite dans le chapitre~\refChLHCCMS, correspond à la somme des impulsions transverses de toutes les particules invisibles de l'événement.
...
\item $L_2$ la partie visible de la désintégration de l'autre lepton \tau, par exemple le tau hadronique résultant de $\Wbosonplus\to\quarku\antiquarkd$ sur la figure~\ref{subfig-chapter-HTT_analysis-section-categorisation-BSM-fgraph-H-tautau_mutau};
...
\nonumber\\
&= 2 \Big[ \pT^{(L_1)}\pT^{(L_2)}(1-\cos\Delta\phi_{L_1,L_2})
\nonumber\\
&\hphantom{=2(} \quad + \pT^{(L_1)}\MET(1-\cos\Delta\phi_{L_1,\MET}) + \pT^{(L_2)}\MET(1-\cos\Delta\phi_{L_2,\MET}) \Big]
\nonumber\\
...
avec
\begin{equation}
\mT^{(A,B)} = \sqrt{2 \, \pT^{(A)} \, \pT^{(B)} \, (1-\cos\Delta\phi_{A,B})}
\mend
\end{equation}
\par
...
{\plotHTTshapes[HTT_v2]{mt_tot}{mssm_classic}{2017}{mt}{35}{prefit_linear_nosignal}}
\hfill
\subcaptionbox{Canal \ele\mu, catégorie \CATbtag\ \CATmediumdz.}[.475\textwidth]
{\plotHTTshapes[HTT_v2]{mt_tot}{mssm_classic}{2017}{em}{36}{prefit_linear_nosignal}}
