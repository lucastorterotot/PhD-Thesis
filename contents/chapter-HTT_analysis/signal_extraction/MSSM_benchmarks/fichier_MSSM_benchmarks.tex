...
\end{table}
\subsection{Cas du scénario avec violation de $CP$}
La violation de la symétrie $CP$,
abordée dans le chapitre~\refChMSSM,
peut également apparaître avec les bosons de Higgs.
Hors violation de $CP$, les bosons sont dits \og $CP$-pairs \fg.
Une composante \og $CP$-impaire \fg{} implique une violation de $CP$.
\par
La présence d'une telle composante peut être directement étudiée par la mesure de l'angle de mixage $CP$ du boson de Higgs \higgs,
$\phi_{CP}^{\higgs}$~\cite{PhysRevD.92.096012}.
Cette mesure peut être réalisée à partir des événements avec une paire de leptons tau~\cite{Bourgatte_thesis},
en particulier,
par les méthodes du paramètres d'impact~\cite{BERGE2013488}, du plan de désintégration~\cite{Desch_2004} et du vecteur polarimétrique~\cite{Cherepanov_cpv},
