...
\bottomrule
\end{tabular}
\caption{Résolutions relatives en \SI{}{\%} de \mTtot\ et \mml\ pour différentes valeurs de $m_{\higgsML}$.}
...
De plus, le signal du boson~\Zboson\ est mieux séparée des événements \ftauhs.
...
\caption{Distributions de \msv\ et \mml\ en 2017 sur les événements de haute masse du canal \tauh\tauh.}
...
\caption{Distributions de \msv\ et \mml\ en 2017 sur les événements de haute masse des canaux \mu\tauh, \ele\tauh\ et \ele\mu.}
...
hormis la queue à haute masse plus important de \mml\ déjà observée.
...
Ces topologies correspondent à certaines catégories de l'analyse du boson de Higgs du modèle standard dans sa désintégration en paire de leptons tau
n'utilisant pas de réseau de neurones pour obtenir une variable discriminante~\cite{sm_categories}.
\par
...
\caption[Distributions de \msv\ et \mml\ en 2017 sur des événements de topologie particulière.]{Distributions de \msv\ et \mml\ en 2017 dans le canal \GetChannelStr{\localch} avec les événements sans jets et tels que la distance $\Delta R$ entre les deux éléments du \emph{dilepton} dans le plan $(\eta,\phi)$ soit inférieure à \num{3.2}.}
...
%{\plotHTTshapes{m_ml}{cut_based_sm}{2017}{\localch}{\tmpcat}{prefit_linear_nosignal_blinded}}
%
%\caption[Distributions de \msv\ et \mml\ en 2017 sur des événements de topologie particulière.]{Distributions de \msv\ et \mml\ en 2017 dans le canal \GetChannelStr{\localch} avec les événements sans jets et tels que la masse transverse de $L_1$ soit comprise entre \num{40} et \SI{70}{\GeV}.}
%\label{fig-svfit_vs_ml_2017-\localch-\tmpcat}
%\end{figure}
%
%\begin{figure}[h]
...
\end{figure}
\par
Notre modèle propose donc des estimations de la masse des particules se désintégrant en paires de leptons tau
...
L'algorithme de \SVFIT\ est connu pour être relativement long en termes d'exécution.
...
compare les temps d'exécution moyens par événements de deux scripts,
...
\par
Les prédictions de notre modèle sont donc 60 fois plus rapides que celles de \SVFIT.
Dans le contexte des prochains Runs du LHC et surtout du HL-LHC,
