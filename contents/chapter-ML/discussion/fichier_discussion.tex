...
La réponse moyenne du modèle est de $\num{1.00}\pm\num{0.05}$ pour $m_{\higgsML}$ entre \SI{80}{\GeV} et \SI{600}{\GeV} avec une résolution à $\pm1\sigma$ de l'ordre de \SI{22}{\%} à basse masse et \SI{10}{\%} à haute masse.
\begin{figure}[h]
\centering
...
ayant les mêmes hyper-paramètres que B mais entraîné en utilisant les objets générés au lieu de ceux reconstruits pour $L_1$, $L_2$ et \MET, \ie\ pour les trois objets physiques liés à la désintégration des leptons tau (deux parties visibles $L_1$ et $L_2$ et \MET\ pour les neutrinos).
...
La résolution relative est toujours de l'ordre de \SI{22}{\%},
...
\par
Dans le canal \tauh\tauh,
la réponse est \SI{10}{\%} plus basse pour \Bchsplit{tt} (figure~\ref{subfig-reponse_model_train_on_tt_test_on_tt}) que pour B (figure~\ref{subfig-reponse_model_train_on_inclusive_test_on_tt})
...
\bottomrule
\end{tabular}
\caption[Résolutions relatives de différents modèles.]{Résolutions relatives minimale et maximale sur des intervalles de \SI{10}{\GeV} pour \Bchsplit{mm}, \Bchsplit{em} ou \Bchsplit{ee} et B.}
...
dans lesquels les quantités de \tauh\ et de neutrinos issus des désintégrations des leptons tau sont constantes.
...
alors que ce n'est la cas que de \SI{200}{\GeV} à \SI{425}{\GeV}
...
et
le premier quantile inférieur (limite basse de la bande verte $\pm1\sigma$) passe de \num{0.8} à \num{1.0}.
\begin{figure}[h]
\centering
...
\end{figure}
\par
Ainsi, $m_{\higgsML}$ est sur-estimée à basse masse
et
...
En termes de classification en un nombre infinie de catégories au lieu de régression, cela revient à dire qu'une famille est donc une catégorie identifiée.
...
\par
Au niveau des bords de l'intervalle en revanche,
figure~\ref{subfig-true_at_pred_750} pour celui de haute masse par exemple,
...
Par conséquent, le DNN ne connaît que les basses masses de cette famille
et la minimisation de la fonction de coût mène à des prédictions plus basses.
Il est donc tout à fait cohérent qu'il sous-estime la masse de ces événements.
Si les événements au-delà de \SI{800}{\GeV} étaient présents dans cette famille,
la distribution apparaîtrait décalée vers la droite, \ie\ vers les hautes masses,
...
\begin{equation}
\Loss \to \Loss' = \LossMAPE \times
\left\lbrace
\begin{aligned}
0 \quad & \text{si le point $(\ytrue, \ypred)$ est dans les zones 3 ou 4}\\
1 \quad & \text{sinon}
\end{aligned}
\right.
\mend
...
sur la figure~\ref{fig-B_Bprime-boundaries_effect}.
\begin{figure}[p]
\centering

\subcaptionbox{Réponse du modèle B à basse masse.\label{subfig-B_lowmass-BvsBprime}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/ML/from_ML_plots/trained_NNs_FastSim/DeepTau-inclusive/PuppiMET_with_METcov_j1j2jr_Nnu_Npu/model_response_lowmass-NN-ADAM_glorot_uniform-activation-softplus-batch_size-2048-mape-Adadelta-u-inclusive-3-layers-1000-neurons.pdf}\vspace{-.5\baselineskip}}
\hfill
\subcaptionbox{Réponse du modèle B' à basse masse.\label{subfig-Bprime_lowmass-BvsBprime}}[.45\textwidth]
...
\subcaptionbox{Réponse du modèle B.\label{subfig-B-BvsBprime}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/ML/from_ML_plots/trained_NNs_FastSim/DeepTau-inclusive/PuppiMET_with_METcov_j1j2jr_Nnu_Npu/model_response-NN-ADAM_glorot_uniform-activation-softplus-batch_size-2048-mape-Adadelta-u-inclusive-3-layers-1000-neurons.pdf}\vspace{-.5\baselineskip}}
\hfill
\subcaptionbox{Réponse du modèle B'.\label{subfig-Bprime-BvsBprime}}[.45\textwidth]
...
\subcaptionbox{Prédictions de B en fonction des valeurs vraies.\label{subfig-B_2dhisto-BvsBprime}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/ML/from_ML_plots/trained_NNs_FastSim/DeepTau-inclusive/PuppiMET_with_METcov_j1j2jr_Nnu_Npu/predicted_vs_answers_histo-NN-ADAM_glorot_uniform-activation-softplus-batch_size-2048-mape-Adadelta-u-inclusive-3-layers-1000-neurons.pdf}\vspace{-.5\baselineskip}}
\hfill
\subcaptionbox{Prédictions de B' en fonction des valeurs vraies.\label{subfig-Bprime_2dhisto-BvsBprime}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/ML/from_ML_plots/trained_NNs_FastSim/DeepTau-inclusive/PuppiMET_with_METcov_j1j2jr_Nnu_Npu/predicted_vs_answers_histo-NN-activation-softplus-batch_size-2048-mapesqrt_b-Adam-gu-inclusive-3-layers-1000-neurons.pdf}\vspace{-.5\baselineskip}}

\caption{Comparaison des modèles B et B'.}
\label{fig-B_Bprime-boundaries_effect}
\end{figure}
Pour des masses inférieures à \SI{70}{\GeV},
la réponse médiane de B', en figure~\ref{subfig-Bprime_lowmass-BvsBprime},
est égale à 1 alors que celle de B, en figure~\ref{subfig-B_lowmass-BvsBprime},
est sujette à l'effet de bord.
À haute masse, 
la réponse de B' en figure~\ref{subfig-Bprime-BvsBprime} est également plus proche de 1
que celle de B, en figure~\ref{subfig-Bprime-BvsBprime}.
...
sur la figure~\ref{fig-BprimevsBpprime-boundaries_effect}.
\begin{figure}[p]
\centering

\subcaptionbox{Réponse du modèle B' à basse masse.\label{subfig-Bprime_lowmass-BprimevsBpprime}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/ML/from_ML_plots/trained_NNs_FastSim/DeepTau-inclusive/PuppiMET_with_METcov_j1j2jr_Nnu_Npu/model_response_lowmass-NN-activation-softplus-batch_size-2048-mapesqrt_b-Adam-gu-inclusive-3-layers-1000-neurons.pdf}\vspace{-.5\baselineskip}}
\hfill
\subcaptionbox{Réponse du modèle B" à basse masse.\label{subfig-Bpprime_lowmass-BprimevsBpprime}}[.45\textwidth]
...
\subcaptionbox{Réponse du modèle B'.\label{subfig-Bprime-BprimevsBpprime}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/ML/from_ML_plots/trained_NNs_FastSim/DeepTau-inclusive/PuppiMET_with_METcov_j1j2jr_Nnu_Npu/model_response-NN-activation-softplus-batch_size-2048-mapesqrt_b-Adam-gu-inclusive-3-layers-1000-neurons.pdf}\vspace{-.5\baselineskip}}
\hfill
\subcaptionbox{Réponse du modèle B".\label{subfig-Bpprime-BprimevsBpprime}}[.45\textwidth]
...
\subcaptionbox{Prédictions de B' en fonction des valeurs vraies.\label{subfig-Bprime_2dhisto-BprimevsBpprime}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/ML/from_ML_plots/trained_NNs_FastSim/DeepTau-inclusive/PuppiMET_with_METcov_j1j2jr_Nnu_Npu/predicted_vs_answers_histo-NN-activation-softplus-batch_size-2048-mapesqrt_b-Adam-gu-inclusive-3-layers-1000-neurons.pdf}\vspace{-.5\baselineskip}}
\hfill
\subcaptionbox{Prédictions de B" en fonction des valeurs vraies.\label{subfig-Bpprime_2dhisto-BprimevsBpprime}}[.45\textwidth]
{\includegraphics[width=.45\textwidth]{\PhDthesisdir/plots_and_images/my_plots/ML/from_ML_plots/selected_NNs_FastSim/DeepTau-inclusive-1TeV/PuppiMET_with_METcov_j1j2jr_Nnu_Npu/plots_up_to_800/predicted_vs_answers_histo-NN-activation-softplus-batch_size-2048-mapesqrt_b-Adam-gu-inclusive-3-layers-1000-neurons.pdf}\vspace{-.5\baselineskip}}

\caption{Comparaison des modèles B' et B".}
\label{fig-BprimevsBpprime-boundaries_effect}
\end{figure}
La réponse à basse masse de B", figure~\ref{subfig-Bpprime_lowmass-BprimevsBpprime},
est semblable à celle de B', figure~\ref{subfig-Bprime_lowmass-BprimevsBpprime},
ce qui est attendu.
En revanche, la réponse moyenne de B" à haute masse, figure~\ref{subfig-Bpprime-BprimevsBpprime},
est de $\num{1.00}\pm\num{0.04}$
contre $\num{0.93}\pm\num{0.07}$ pour B', figure~\ref{subfig-Bprime-BprimevsBpprime}.
