...
Les corrections décrites dans les sections précédentes permettent d'obtenir une bonne calibration des jets.
...
\end{equation}
\end{description}
\subsubsection{Corrections résiduelles relatives en $\eta$}
...
\caption{Correction résiduelle relative en $\eta$ pour $\pT^\text{jet}=\SI{120}{\GeV}$ lors du Run~II obtenue avec des événements dijets et la méthode MPF~\cite{CMS-DP-2020-019}.}
\label{fig-L2ResRel_RunII}
\end{figure}
\subsubsection{Corrections résiduelles absolues en \pT}
...
\subsection{Corrections dépendantes de la saveur}
...
As explained, the MC-truth based corrections are extracted for a QCD flavor mixture sample. Optional jet-flavor corrections are derived from MC simulation, using Z+jet and photon+jets simulated events. These corrections are provided for the Z+jet and photon+jets mixtures, and also for pure flavors (to be applied to analysis-specific mixtures). JEC flavor uncertainties (see the JEC uncertainty twiki) are also extracted, and can be applied to data vs simulation comparisons regardless of whether or not the jet-flavor corrections are applied in the analysis. The flavor corrections and their uncertainties for b-quark jets are checked in data with Z+b events. 
...
\subsection{Résumé des corrections}

\begin{equation}
\pT_\cali =
\underbrace{\underbrace{\pT_\reco^\text{CHS}
\times
\mathcal{C}_\text{PU}(\pT_\reco^\text{CHS}, \eta, A_j, \rho)}_{\pT_\reco'}
\times
\mathcal{C}_\text{Rép}(\pT_\reco', \eta)}_{\pT_\reco''}
\times
\mathcal{C}_\text{Res}(\pT_\reco'', \eta)
\end{equation}