...
Alors, toutes les traces associées aux vertex d'empilement sont retirées de l'événement.
...
Il s'agit d'une correction paramétrique dépendant de:
\begin{itemize}
\item la densité en énergie de l'événement, $\rho$;
\item l'aire du jet dans le plan $(\eta, \phi)$, $A$;
\item la pseudo-rapidité du jet, $\eta$;
\item l'impulsion transverse du jet avant application de la correction, \ie\ à cette étape $\pT_\reco$\\\todo{ben non, tu as déjà une correction CHS! propage la correction ...}.
\end{itemize}
La correction $C$ à appliquer à un jet s'exprime alors
\begin{equation}
C(\pT_\reco, \eta, A, \rho) = 1 - \frac{\average{\pT_\ptcl^\text{add}}}{\pT_\reco}
\end{equation}
où $\average{\pT_\ptcl^\text{add}}$ représente l'effet additionnel de l'empilement au niveau particule, estimé à partir d'événements di-jet simulés avec et sans empilement, tel que
\begin{equation}
\average{\pT_\ptcl^\text{add}}
=
\average{\pT_\ptcl^\text{avec PU} - \pT_\ptcl^\text{sans PU}}
\mend[,]
\end{equation}
avec
$\pT_\ptcl^\text{avec PU}$
l'impulsion au niveau particule avec empilement
et
$\pT_\ptcl^\text{sans PU}$
l'impulsion au niveau particule sans empilement.
