...
Un des principaux bruits de fond provient d'événements multijet dans lesquels l'un des jets est identifié à tort comme un photon.
...
%La figure~\ref{fig-chapter-JERC-section-pheno-GJets-photon_resolution} montre en effet que ces photons possèdent une résolution relative en énergie de l'ordre de \SI{1}{\%}, contre environ \SI{2.5}{\%} pour les photons des \CMSendcaps.
...
Dans cette analyse, seuls les événements dont le photon servant d'objet de référence pour la calibration des jets correspond à celui ayant activé le \HLTpath\ 
%pour cet événement
sont retenus.
Or, plus l'impulsion transverse du photon $\pT^{\photon}$ est faible, plus le nombre d'événements pouvant être sauvegardés est importante, si bien que la chaîne d'acquisition arrive à saturation.
Pour pallier cela, il existe différents \HLTpaths\ selon $\pT^{\photon}$. Pour chacun d'entre eux, seule une fraction des événements les déclenchant est sauvegardée,
déterminée par le nombre d'événements observés pour un sauvegardé ou \og \emph{prescale} \fg.
Un \emph{prescale} de \num{100} signifie qu'un événement sur \num{100} est enregistré.
%À chaque \HLTpath\ correspond ainsi un \emph{prescale}.
Un intervalle en $\pT$ du photon retenu est défini pour chaque \HLTpath\ utilisé afin de se placer à son plateau d'efficacité.
Le tableau~\ref{tab-HLT_pT_precales_18_and_17UL} résume ces intervalles et \emph{prescales}.
Par exemple, un photon tel que $\pT^{\photon} = \SI{95}{\GeV}$ doit avoir activé le chemin nommé \inlinecode{python}{HLT_Photon75_R9Id90_HE10_IsoM}.
\begin{table}[h]
\centering
\begin{tabular}{lccc}
\toprule
...
Les événements simulés sont ainsi pondérés pour faire correspondre leur profil d'empilement à celui des données réelles, en prenant en compte la double dépendance avec la période de prise de données et le \HLTpath.
\begin{figure}[p]
\centering
...
La présence d'un jet secondaire, comme sur la figure~\ref{subfig-Gamma_plus_two_jets}, crée un déséquilibre dans \Rbal\ dû à la physique de l'événement et non à la JES. Il ne faut donc pas corriger cet effet.
...
\begin{figure}[h]
\centering
\subcaptionbox{Réponse équilibrée.\label{subfig-chapter-JERC-section-JES-subsec-analyse-response_eta0013_ptPhot_175_230_extrap}}[.45\textwidth]
...
\input{\PhDthesisdir/contents/chapter-JERC/JES/figures_shortcuts/responses_balancing_alpha_0_3_2018ABCD.tex}
\input{\PhDthesisdir/contents/chapter-JERC/JES/figures_shortcuts/responses_MPF_alpha_0_3_2018ABCD.tex}
\input{\PhDthesisdir/contents/chapter-JERC/JES/figures_shortcuts/responses_balancing_extrapolated_2018ABCD.tex}
\input{\PhDthesisdir/contents/chapter-JERC/JES/figures_shortcuts/responses_MPF_extrapolated_2018ABCD.tex}
...
\input{\PhDthesisdir/contents/chapter-JERC/JES/figures_shortcuts/responses_recap_table.tex}
...
\par L'ajustement global sur les résultats des différentes analyses est illustré, pour les trois années du Run~II, sur la figure~\ref{fig-L3ResAbs_RunII}.
...
\end{figure}
\input{\PhDthesisdir/contents/chapter-JERC/JES/figures_shortcuts/responses_recap_table_L2L3Res_cross_check.tex}
\begin{figure}[p]
\centering
